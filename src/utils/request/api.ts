 ...
 * @returns
 */
export const searchMusicApi: (name: string, limit?: number, offset?: number) => any = (
  name,
  limit = 25,
  offset = 0,
) => {
  return musicSourceActuator(
        ...
        .then((res: any) => {
          ...
          res.result.songs.forEach((item: any) => {
            const musicObj: MusicType = {
              musicName: item.name,
              musicId: item.id,
              singerName: item.artists.reduce(
                (value: string, item: any, index: number) =>
                  `${item.name}${index > 0 ? '、' : ''}${value}`,
                '',
              ),
    ...
    () => {
      return http.get(`search?keywords=${name}`).then(() => ({}));
    },
  );
};
/**
  ...
  return musicSourceActuator(
    () =>
      http.get(`search/suggest?keywords=${value}`).then((res: any) => {
        ...
        const artists = res.result.artists?.map((item: any) => ({
          id: item.id,
          singerName: item.name,
        }));
        const albums = res.result?.albums.map((item: any) => ({
          id: item.id,
          albumName: item.name,
  ...
  return musicSourceActuator(
    () =>
      http
        ...
        .catch(() => message.error('当前网络不佳，请检查网络是否有效。')),
    () =>
      http
        .get(`playlist/detail?id=${id}`)
        .then((res: any) => {
